...
def original_func():
	if request.method=='POST':
		...
		if expected_sent == "Positive":
			expected_sent1 = 1
		elif expected_sent == "Negative":
			expected_sent1 = 0
		elif expected_sent =="Neutral":
			expected_sent = 0.5
	...
	content2 = len(content1.split())
	flag1 = 0
	max_word = 150
	if max_word >= len(content1.split()):
		flag1 = 2
	
	stopwords1 = ['what', 'who', 'is', 'a', 'at', 'is', 'he', 'in', 'on', 'write', 'an', 'essay', 'on', 'detailed', 'detail', 'note', 'brief']
...
#type(errors[0])
	...
	elif expected_sent1 == 0 and sent == 1:
	...
	elif expected_sent1 == 0.5:
		sent_op = "The sentiment is relevant and accepted\t1"
	...
	field_names = ['essay_set','essay', 'rater1_domain1', 'rater2_domain1', 'rater3_domain1','domain1_score']
	add_dict = {'essay_set':0,'essay':content, 'rater1_domain1':0, 'rater2_domain1':final_score,'rater3_domain1':0, 'domain1_score':final_score}
	with open('dataset.tsv', 'a') as f_object:
		dictwriter_object = DictWriter(f_object, fieldnames=field_names, delimiter = '\t')
		dictwriter_object.writerow(add_dict)
		f_object.close()
	...
	if flag == 1:
		dict1 = {"topic":query, "original":content,"errors" : errors1 , "sentiment" : sent1, "plagiarism" : plag, "total_plagiarism" : total, "Quality_of_Essay" : a1, "spell_error" : b, "plagiarism_marks_lost" : c, "sentiment_op" : sent_op, "final_grade" : final_score, "comment" : y, "corrected_essay" : new_text}
		...
		final_score = 2
		dict1 = {"topic":query, "original":content,"errors" : errors1 , "sentiment" : sent1, "plagiarism" : plag, "final_grade" : final_score, "comment" : y}

	if flag1 == 2:
		y = "Poor! Your essay is considerably short"
